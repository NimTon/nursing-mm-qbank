...
def preprocess_image(
    ...
    perspective: dict[str, Any] | bool | None = None,
) -> Path:
    """
    ``max_long_edge``:
    ...
    if ocr_opts.get("enabled"):
        try:
            img, _k, _om = auto_rotate_upright_pulc_bgr(
                img,
    ...
    try:
        img, pmeta = apply_perspective_bgr(img, perspective)
        perspective_applied = bool(pmeta.get("applied"))
    except Exception:  # noqa: BLE001
        perspective_applied = False
    ...
    if on and not perspective_applied:
